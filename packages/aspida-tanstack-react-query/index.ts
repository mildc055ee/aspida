...
>(api: T, key: U, ...option: QueryOptions<T[U]>): QueryResult<T[U]> //TODO: replace key to keys
function useAspidaQuery<
  T extends Record<string, any> & { $path: (option?: any) => string },
  U extends { [K in keyof T]: T[K] extends (option: any) => Promise<any> ? K : never }[keyof T]
>(api: T, key: U, ...option: Parameters<T[U]>) {
  ...
  return useQuery(
    typeof key === 'string' ? [api.$path(opt), method] : api.$path(opt), //TODO: query key is replaced with query keys
    () => api[method](opt),
    opt
  )
}
